...
class ModelParams(ParamGroup): 
    ...
    def extract(self, args): #类的成员方法（第一个是self，代表类的实例对象，第二个是args，是一些参数。）
        g = super().extract(args) # 调用父类的extract方法，并传入参数args。这行代码将父类的extract方法返回的结果赋值给了变量g。
...
class PipelineParams(ParamGroup):
    def __init__(self, parser):
        self.convert_SHs_python = False     # 如果提供了预先计算的颜色
        self.compute_cov3D_python = False   # 是否提供了预先计算的3D协方差矩阵
        self.debug = False
