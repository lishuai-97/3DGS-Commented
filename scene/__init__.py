    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if not self.loaded_iter:
            with open(scene_info.ply_path, 'rb') as src_file, open(os.path.join(self.model_path, "input.ply") , 'wb') as dest_file:
                dest_file.write(src_file.read())    # 复制PLY文件
            json_cams = []
            ...
            for id, cam in enumerate(camlist):   # Rwc和tcw
                json_cams.append(camera_to_JSON(id, cam))   #  按顺序进行JSON转换并添加到json_cams，无用
        ...
        # 加载训练和测试相机：
        for resolution_scale in resolution_scales: #可选参数，默认为 [1.0]。一个浮点数列表，用于指定相机的分辨率缩放因子
