...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    scene = Scene(dataset, gaussians) #（这个类的主要目的是处理场景的初始化、保存和获取相机、点云、pose信息等）创建一个 Scene 类的实例，使用数据集和之前创建的 GaussianModel 实例作为参数。
    ...
    for iteration in range(first_iter, opt.iterations + 1): #主要的训练循环开始。       
        ...
        gaussians.update_learning_rate(iteration) #更新xyz坐标的学习率

        # Every 1000 its we increase the levels of SH up to a maximum degree
        if iteration % 1000 == 0:
            gaussians.oneupSHdegree() #每 1000 次迭代，增加球谐函数的阶数，最大为3，初始为0。

        # Pick a random Camera （随机选择一个训练相机），NOTE：这里选择了随机的一个训练相机，也就是说后续的渲染均基于这个相机的视角
        ...
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) # loss公式，L1和LD-ssim的均值
        loss.backward() # 更新损失。loss反向传播（pytorch的标准调用）
        ...
        with torch.no_grad(): # 记录损失的指数移动平均值，并定期更新进度条。
            ...
            if iteration < opt.densify_until_iter: #在达到指定的迭代次数之前执行以下操作。
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0: # 在指定的迭代次数之后，每隔一定的迭代间隔进行以下密集化操作
                    size_threshold = 20 if iteration > opt.opacity_reset_interval else None # 根据当前迭代次数设置密集化的阈值
