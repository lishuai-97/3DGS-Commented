...
__device__ glm::vec3 computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, bool* clamped)
{
	...
	glm::vec3 dir = pos - campos;	// 向量
	dir = dir / glm::length(dir);	// 观察方向，方向向量
...
__device__ float3 computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float* cov3D, const float* viewmatrix)
{
	...
	glm::mat3 J = glm::mat3(	// 雅可比矩阵（用泰勒展开近似）
...
__device__ void computeCov3D(const glm::vec3 scale, float mod, const glm::vec4 rot, float* cov3D)
{
	...
	float r = q.x;	// 这里的x其实是rot里面的 w，rot顺序：wxyz
	float x = q.y;
	float y = q.z;
	float z = q.w;
	...
	glm::mat3 Sigma = glm::transpose(M) * M;	// 论文公式6：R S ST RT，这里却是 RT ST S R
												// 这里同样是因为 glm是按列优先，mat[i][j]读取的是第i列第j行
												// 代码中的 R 对应论文的 RT...
...
__global__ void preprocessCUDA(
	int P,  // 高斯分布的点的数量
	int D,  // 对应于GaussianModel.active_sh_degree，是当前活动的球谐阶数
	int M, 	// RGB三通道的球谐系数个数，(最大球谐阶数 + 1)²
	const float* orig_points, 		// Gaussians的中心坐标, (N，3)
	const glm::vec3* scales, 		// 缩放
	const float scale_modifier, 	// 缩放调整因子
	const glm::vec4* rotations, 	// 旋转
	const float* opacities, 		// 不透明度，N
	const float* shs, 				// 球谐系数, (N, (最大球谐阶数 + 1)², 3)
	bool* clamped, 					// 用于记录是否被裁剪
	const float* cov3D_precomp, 	// 预计算的三维协方差
	const float* colors_precomp, 	// 预计算的颜色
	const float* viewmatrix, 		// 世界到相机坐标系的变换矩阵的转置, W^T，4*4
	const float* projmatrix, 		// 投影矩阵，世界坐标系到NDC系的变换矩阵的转置，J^T W^T
	const glm::vec3* cam_pos, 		// 相机位置
	const int W, int H, 			// 图像的宽度和高度
	const float tan_fovx, float tan_fovy, // 水平和垂直方向的焦距正切
	...
	float* rgb, 					// 输出的颜色
	float4* conic_opacity, 			// 2D 协方差矩阵的逆矩阵3参数的矩阵和不透明度的打包向量
	const dim3 grid, 				// x和y方向上tile的个数
	...
	bool prefiltered)
{
	...
	if (!in_frustum(idx, orig_points, viewmatrix, projmatrix, prefiltered, p_view))
		return;	// 不在相机的视锥内就不管了（这里其实只筛选了深度，后续可以改进）

	// Transform point by projecting
	// 对原始点进行投影变换，计算其在屏幕上的坐标
	float3 p_orig = { orig_points[3 * idx], orig_points[3 * idx + 1], orig_points[3 * idx + 2] };
	float4 p_hom = transformPoint4x4(p_orig, projmatrix);	
	float p_w = 1.0f / (p_hom.w + 0.0000001f);	// p_hom.w 是GS投影到成像平面后的深度z
	float3 p_proj = { p_hom.x * p_w, p_hom.y * p_w, p_hom.z * p_w };  // GS投影到成像平面上的坐标 (x/z, y/z, 1)
	...
	float det = (cov.x * cov.z - cov.y * cov.y);	// 二维协方差矩阵的行列式，也为特征值之积
	if (det == 0.0f)
		return;
	float det_inv = 1.f / det;	// 行列式的逆
	float3 conic = { cov.z * det_inv, -cov.y * det_inv, cov.x * det_inv };	
		// 而为协方差矩阵的逆矩阵，主对调，次换号，除以行列式

	// Compute extent in screen space (by finding eigenvalues of
	// 2D covariance matrix). Use extent to compute a bounding rectangle
	// of screen-space tiles that this Gaussian overlaps with. Quit if
	// rectangle covers 0 tiles. 
	// 韦达定理计算2D协方差矩阵的特征值，用于计算屏幕空间的范围，以确定与之相交的瓦片。
	float mid = 0.5f * (cov.x + cov.z);	// 平均特征值
	float lambda1 = mid + sqrt(max(0.1f, mid * mid - det));	// https://blog.csdn.net/weixin_48524215/article/details/124355663
	float lambda2 = mid - sqrt(max(0.1f, mid * mid - det));
	float my_radius = ceil(3.f * sqrt(max(lambda1, lambda2)));	// 半径选择最大的特征值，椭圆近似为圆，3σ原则
	float2 point_image = { ndc2Pix(p_proj.x, W), ndc2Pix(p_proj.y, H) };  // ndc到像素平面的转换，GS的像素坐标
						// ndc2Pix(v, S) = ((v + 1) * S - 1) / 2 = (v + 1) / 2 * S - 0.5
	uint2 rect_min, rect_max;
	// 检查该Gaussian在图片上覆盖了哪些tile（由一个tile组成的矩形表示）
	getRect(point_image, my_radius, rect_min, rect_max, grid);
	if ((rect_max.x - rect_min.x) * (rect_max.y - rect_min.y) == 0)
		return;  // 不与任何tile相交，不计算这个GS
	...
	// 储存计算得到的深度、半径、屏幕坐标等结果，用于下一步继续处理。
	...
	conic_opacity[idx] = { conic.x, conic.y, conic.z, opacities[idx] };// 2D 协方差矩阵的逆矩阵3参数以及点的不透明度值打包
	tiles_touched[idx] = (rect_max.y - rect_min.y) * (rect_max.x - rect_min.x);	// Gaussian覆盖的tile数量
}
...
renderCUDA(
	...
	const float4* __restrict__ conic_opacity, 	// 椭圆2DCOV的逆矩阵3参数和不透明度的打包向量
	...
	float* __restrict__ out_color) 				// 渲染结果（图片）
{
	...
	{	
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) //在当前批次的循环中，每个线程处理一条点云数据。
		{
			...
			float2 d = { xy.x - pixf.x, xy.y - pixf.y };	// 该像素到Gaussian中心的位移向量，（x-miu）
			...
			float alpha = min(0.99f, con_o.w * exp(power));	// 当前Gaussian对于这个像素点来说的不透明度
			if (alpha < 1.0f / 255.0f)	 // 不透明度太小，不渲染
				continue;
			float test_T = T * (1 - alpha);	// 透光率累乘
			if (test_T < 0.0001f)	// 不透明度接近1（饱和）
			{
				done = true;
				continue;
			}
			...
			for (int ch = 0; ch < CHANNELS; ch++)
				C[ch] += features[collected_id[j] * CHANNELS + ch] * alpha * T;	// alpha blending的核心公式
