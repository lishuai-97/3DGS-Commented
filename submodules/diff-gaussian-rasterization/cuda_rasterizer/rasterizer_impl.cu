...
__global__ void duplicateWithKeys(
	...
	dim3 grid)
{
	auto idx = cg::this_grid().thread_rank();	// 当前处理GS的id
	if (idx >= P)
		return;
	...
	if (radii[idx] > 0)
	{
		...
		for (int y = rect_min.y; y < rect_max.y; y++)
		{
			for (int x = rect_min.x; x < rect_max.x; x++)
			{
				...
				gaussian_values_unsorted[off] = idx;	// value是GS的idx
				off++;
			}
		}
	}
	...
	)
{
	...
	else
	{
		...
		if (currtile != prevtile)	// 上一个元素和我处于不同的tile，
...
int CudaRasterizer::Rasterizer::forward(
	...
	*/
	...
	const float* background,		// 背景颜色，默认1*3：[0, 0, 0]
	const int width, int height,	// 图像宽高
	const float* means3D,			// Gaussians的中心坐标, (N，3)
	const float* shs,				// 球谐系数, (N, (最大球谐阶数 + 1)², 3)
	const float* colors_precomp,	// 预计算的颜色
	const float* opacities,			// 不透明度，（N，1）
	const float* scales,			// 缩放，(N，3)，ln
	const float scale_modifier,		// scale_modifier，1
	const float* rotations,			// 旋转四元数，（N，4）
	const float* cov3D_precomp,		// 预计算的3D协方差
	const float* viewmatrix,		// 世界到相机坐标系的变换矩阵的转置, W^T，4*4
	const float* projmatrix,		// 世界坐标系到图像的变换矩阵的转置, W^T J^T，4*4
	const float* cam_pos,			// 相机在世界坐标系下的坐标，twc，1*3
	const float tan_fovx, float tan_fovy,	// 视场角一半的正切值
	const bool prefiltered,	// false
	float* out_color,		// 输出的颜色，（3，H，W）
	int* radii,				// 各Gaussian在像平面上用3σ原则截取后的半径，N，注意这里近似为了圆，而不是椭圆
	bool debug)
{
...
*/
	...
	dim3 tile_grid((width + BLOCK_X - 1) / BLOCK_X, (height + BLOCK_Y - 1) / BLOCK_Y, 1); // x和y方向上tile的个数
	// BLOCK_X = BLOCK_Y = 16，准备分解成16×16=256 pixels的tiles。
	...
	// 预处理，把3D的Gaussian投影到2D
	CHECK_CUDA(FORWARD::preprocess(
		P,		// 高斯分布的点的数量
		D, 		// 对应于GaussianModel.active_sh_degree，是当前活动的球谐阶数
		M,		// RGB三通道的球谐系数个数，(最大球谐阶数 + 1)²
		means3D,
		(glm::vec3*)scales,
		scale_modifier,
		(glm::vec4*)rotations,
		...
		tan_fovx, tan_fovy,
		radii,				// 输出的半径
		geomState.means2D,	// 输出的Gaussian投影到像平面上的中心坐标
		geomState.depths,	// 输出的Gaussian的深度
		geomState.cov3D,	// 输出的三维协方差矩阵
		geomState.rgb,		// 输出的颜色
		geomState.conic_opacity,	// 输出的椭圆2DCOV的逆矩阵3参数和不透明度的打包向量
		tile_grid,					// x和y方向上tile的个数
		geomState.tiles_touched,	// Gaussian覆盖的tile数量
		prefiltered
	), debug)
	...
	CHECK_CUDA(cub::DeviceRadixSort::SortPairs(	// device级别的并行基数排序
	...
	bool debug)
{
	...
	const dim3 tile_grid((width + BLOCK_X - 1) / BLOCK_X, (height + BLOCK_Y - 1) / BLOCK_Y, 1);	// x和y方向上tile的个数
