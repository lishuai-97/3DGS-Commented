...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        viewmatrix=viewpoint_camera.world_view_transform,   # 世界到相机坐标系的变换矩阵的转置, W^T
        projmatrix=viewpoint_camera.full_proj_transform,    # 从世界坐标系到图像的变换矩阵的转置, W^T J^T
        sh_degree=pc.active_sh_degree,          # 当前的激活的球谐阶数    
        campos=viewpoint_camera.camera_center,  # 相机在世界坐标系下的坐标，twc
        prefiltered=False,                      # ？
        debug=pipe.debug
    )
